...
angular.module('Voices', [])
  .constant('Call', (function() {
    ...
    function Call(elements, origin, dest) {
      ...
      this.fromSector = elements[11];
      this.toSector = elements[14];
  ...
  .directive('ttCallMap', function(VoicesService) {
    var svg, projection, points, index, scope, circles,
        line = d3.svg.line()
            .x(function(d) { return projection(d)[0]; })
            .y(function(d) { return projection(d)[1]; });

    var interactionColors = d3.scale.category20();
    var sectors = ['Agriculture', 'Manufacturing', 'Construction', 'Transport',
    ...
    function updateSectorData(slice) {
      for (var i = 0; i < interactionMatrix.length; i++) {
        interactionMatrix[i] = 0;
      }

      for (var i = 0; i < slice.length; i++) {
        var call = slice[i];
        interactionMatrix[4*sectors.indexOf(call.fromSector) + sectors.indexOf(call.toSector)]++;
      }
    }
